...
`Layout.jsx` renders a full-width storefront-style shell: a top bar (hamburger, brand, search, notification/cart/account icons), a left sidebar, the routed page in the middle, and - on wide viewports, off the `/chat` route - a docked **AI Assistant** panel on the right. `#root:has(> .storefront-shell)` zeroes out `#root`'s own padding/centering for exactly this shell, since that padding is still correct for `VerifyPage`'s separate, narrower `.app-shell--centered` card.
