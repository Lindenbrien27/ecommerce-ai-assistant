...
`Layout.jsx` renders a full-width storefront-style shell: a top bar (hamburger, brand, search, notification/cart/account icons) and a left sidebar (a greeting, a nav list, a "Need Help?" card, Logout) - `#root:has(> .storefront-shell)` zeroes out `#root`'s own padding/centering for exactly this shell, since that padding is still correct for `VerifyPage`'s separate, narrower `.app-shell--centered` card.

Most of this chrome is intentionally inert: plain `<span>`s, not `<button>`s/`<a>`s, with no click handler. This app has no catalog, wishlist, address-book, or dashboard backend for Home, Shop, Categories, Wishlist, My Account, Saved Address, or Change Password to actually do something - giving them real interactive semantics with no real behavior would mean either a broken keyboard/AT experience (focusable, announced as a control, does nothing) or fighting that with `tabIndex`/`aria-hidden` on every one. A non-interactive element styled to look clickable is simpler and more honest about what it actually is. **My Orders**, **Support Chat**, and **Logout** are the three that are real `NavLink`/`button` elements - Logout specifically was kept working even though it isn't part of the reference design this shell's style is based on, since a purely decorative Logout would strand anyone signed in with no way back to `/verify`. (Home was originally a fourth real link, pointed at the same `/orders` route as My Orders - removed once that meant *both* lit up "active" at the same time, which reads as a bug, not a feature, and this app genuinely has no separate home/dashboard page for it to point to instead.)

The order list's status filter (All/On shipping/Arrived/Cancelled/Returned) *is* fully functional, unlike the chrome above - it's a client-side filter over already-loaded orders plus real counts computed per status, not decoration, since the data to back it honestly was already there. The one exception is **Returned**: this app's `status` CHECK constraint has no matching value, so that filter's count is always 0 and its list is always empty rather than being quietly mapped onto some other status that isn't actually the same thing.
