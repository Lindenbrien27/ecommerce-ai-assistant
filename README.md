...
`Layout.jsx` renders a full-width storefront-style shell: no separate top bar - the brand sits at the top of the left sidebar, the (decorative) search bar and the real theme toggle live in the main content card's own header, a bottom-of-sidebar profile chip replaces where an account menu used to live in a top bar, and - on wide viewports, off the `/chat` route - a docked **AI Assistant** panel sits on the right. All three columns (sidebar, content, AI panel) are their own boxed card (`--color-bg` surface, a soft border, radius) with real gaps between them and the shell's own ambient `--color-page-bg`, rather than a shared header plus edge-to-edge columns. `#root:has(> .storefront-shell)` zeroes out `#root`'s own padding/centering for exactly this shell, since that padding is still correct for `VerifyPage`'s separate, narrower `.app-shell--centered` card.

The palette is strictly monochrome - no blue or any other hue anywhere in `index.css` - and deliberately soft: light mode's `--color-border` (`#e5e5e8`) and `--color-page-bg` (`#f4f4f6`) are both light, low-contrast grays, so the three cards read as a subtle separation from the page rather than a heavy outline. `--color-solid-bg`/`--color-primary`/`--color-primary-text` (primary buttons, the order filter's active tab, the user's own chat bubble) are built from two anchor tones, `#171717` (near-black) and `#fbfbfb` (near-white), that swap roles per theme: light mode's active fill is the near-black tone on near-white text, dark mode's is the reverse - so an active/primary element is usually the strongest, most contrasting thing on the page rather than just another gray. The sidebar nav is the one deliberate exception - `:hover` and `.active` share the same subtle `var(--color-surface)` fill (`#eaeaea` light / `#1f1f1f` dark) rather than active being an inverted solid pill, so the currently-viewed page reads as a soft highlight instead of competing with real primary actions for visual weight. `.order-card` has an explicit `background: var(--color-bg)` since its parent (`.storefront-content`) doesn't supply one - it's a boxed card sitting on the page background, not the other way around. The AI panel's icon+name header is a real toggle button (`aria-expanded`) - clicking it collapses the panel to a narrow icon rail (content/`.ai-panel-log`/`.ai-panel-form` simply aren't rendered while minimized) and clicking it again reopens it.

The sidebar nav's count badges (`.storefront-sidenav-count`) sit on their own dedicated `--color-nav-badge-bg` token (solid white light / `#1a1a1a` dark) rather than reusing `--color-surface`, so they still read as a distinct chip against the row's own now-identical hover/active background. Getting both the badge's smaller font size and its background to actually apply took extra specificity, twice: `.storefront-sidenav a, .storefront-sidenav span` and the `:hover`/`.active` "…span" variants (added earlier to fix a different contrast bug - see below) all match this badge too, since it's itself a `<span>` nested inside the same row, and a class+type selector beats a single class regardless of which rule was written more recently. Both the base badge rule and its `:hover`/`.active` overrides needed scoping up to compete - found live via computed styles disagreeing with what the CSS visibly said, not assumed.

The order status filter (`.order-filter-tabs`) is a single connected segmented control - one shared background track, not individually-bordered pill buttons - with the active item getting an elevated solid fill and inactive items sitting borderless directly on the track. Both the active-tab count badge here and the active-nav-item count badge in the sidebar use a neutral `rgba(128, 128, 128, …)` overlay rather than a white-tinted one - a white overlay is invisible once dark mode's active fill is itself near-white, a real bug this design pass caught and fixed rather than just matched from the reference.

Light/dark defaults to the OS's own `prefers-color-scheme`, same as before, but a real sun/moon button (in the content card's header now, not a top bar - see `useTheme.js`) lets it be overridden manually - useful for actually seeing both themes without going into system settings. The override is a `data-theme="light"|"dark"` attribute on `<html>`, persisted to `localStorage`; `index.css` declares each theme's variables three times - the default `:root` block, the `@media (prefers-color-scheme: dark)` block (now scoped to `:root:not([data-theme="light"])` so an explicit light override can suppress it), and an unconditional `:root[data-theme="dark"]` block (an attribute selector, so it outranks the plain `:root` a system-light default lives on) - the standard "respect system preference, but let a manual toggle win" pattern, which needs that duplication since a plain CSS rule can't be shared across a `@media` boundary.

The sidebar is grouped into a **Dashboard** section (My Orders/Coupons/Wishlist) and a **Category** section (Audio/Cables/Peripherals/Furniture/Displays - this app's own real product types, not the clothing categories in the reference mockup this layout is modeled on, since this store doesn't sell clothes), plus FAQ's/Settings/Profile below. The Category tags are the one deliberate spot of color in an otherwise strictly monochrome palette - each renders as a low-opacity colored badge rather than a flat gray one. That took two RGB shades per tag, not one: `aria-hidden="true"` (the whole section isn't a real filter) exempts it from screen readers but *not* from WCAG contrast, since a low-vision sighted user still has to read it visually, and a single mid-saturation hue (violet-500, say) can't hit 4.5:1 against its own translucent tint in both themes at once - that same tint renders as a pale wash on light mode's near-white cards (needing a dark shade of the hue for contrast) but a deep wash on dark mode's near-black ones (needing a light shade instead). Found by axe (2.3:1-3.4:1 against a fixed mid-tone the first time through, well under the minimum), not assumed - fixed by giving each tag an 800-level shade for light mode and a 200-level shade for dark mode, switched via the same `:root[data-theme]`/`prefers-color-scheme` pattern the base palette uses. Most of this is intentionally inert: plain `<span>`s, not `<button>`s/`<a>`s, with no click handler. This app has no coupon-management, wishlist, FAQ, or settings/profile-editing backend for those to actually do something - giving them real interactive semantics with no real behavior would mean either a broken keyboard/AT experience (focusable, announced as a control, does nothing) or fighting that with `tabIndex`/`aria-hidden` on every one. A non-interactive element styled to look clickable is simpler and more honest about what it actually is. **My Orders**, **Support Chat**, **Logout**, and the **AI Assistant panel** are the ones that are real - Logout specifically was kept working even though it isn't part of the reference design this shell's style is based on, since a purely decorative Logout would strand anyone signed in with no way back to `/verify`. My Orders' and Coupons' sidebar counts are real numbers derived from the signed-in customer's own orders (order count; orders with a voucher applied); Wishlist has no backing data at all and gets no count rather than a fabricated one.
...
- **The order-progress "milestone" pulse respects `prefers-reduced-motion`** - the current step's icon/dot (`.order-progress-icon` on `OrdersPage`, `.status-timeline-marker` on `OrderDetailPage`) gets a one-shot glow-and-scale pulse (`step-pulse`, 0.9s, `box-shadow` + `transform` only - never `opacity`/`color`, so it can't create a transient contrast dip the way an earlier animation in this file once did) on mount, acknowledging whatever status the order is *already* at rather than a live event. Getting the reduced-motion override to actually win took two attempts: a same-specificity `animation: none` rule that's earlier in the file than the `animation: step-pulse …` rule it's meant to override loses on source order despite being conditional, so it has to be moved to a second `@media (prefers-reduced-motion: reduce)` block placed after both animated rules - caught live (`animation-name` still read `step-pulse` under an emulated reduced-motion context) rather than assumed correct from the CSS alone.
