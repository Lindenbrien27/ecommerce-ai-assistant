...
The palette is strictly monochrome - no blue or any other hue anywhere in `index.css` - and deliberately soft: light mode's `--color-border` (`#e5e5e8`) and `--color-page-bg` (`#f4f4f6`) are both light, low-contrast grays, so the three cards read as a subtle separation from the page rather than a heavy outline. `--color-solid-bg`/`--color-primary`/`--color-primary-text` (active nav pill, primary buttons, the user's own chat bubble) are built from two anchor tones, `#171717` (near-black) and `#fbfbfb` (near-white), that swap roles per theme: light mode's active fill is the near-black tone on near-white text, dark mode's is the reverse - so an active/primary element is always the strongest, most contrasting thing on the page rather than just another gray. `.order-card` has an explicit `background: var(--color-bg)` since its parent (`.storefront-content`) doesn't supply one - it's a boxed card sitting on the page background, not the other way around. The AI panel's icon+name header is a real toggle button (`aria-expanded`) - clicking it collapses the panel to a narrow icon rail (content/`.ai-panel-log`/`.ai-panel-form` simply aren't rendered while minimized) and clicking it again reopens it.
