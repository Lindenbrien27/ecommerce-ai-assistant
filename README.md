...
The palette is strictly monochrome - no blue or any other hue anywhere in `index.css`. `--color-solid-bg`/`--color-primary`/`--color-primary-text` (active nav pill, primary buttons, the user's own chat bubble) are built from two anchor tones, `#171717` (near-black) and `#fbfbfb` (near-white), that swap roles per theme: light mode's active fill is the near-black tone on near-white text, dark mode's is the reverse - so an active/primary element is always the strongest, most contrasting thing on the page rather than just another gray. The sidebar and AI panel are boxed cards (`--color-bg` surface, border, radius) with real gaps between them and the page's own ambient `--color-page-bg`, rather than the previous edge-to-edge columns separated by a single 1px border; `.order-card` picked up an explicit `background: var(--color-bg)` for the same reason, since its parent (`.storefront-content`) no longer supplies one. The AI panel's icon+name header is a real toggle button (`aria-expanded`) - clicking it collapses the panel to a narrow icon rail (content/`.ai-panel-log`/`.ai-panel-form` simply aren't rendered while minimized) and clicking it again reopens it.

Light/dark defaults to the OS's own `prefers-color-scheme`, same as before, but a real sun/moon button in the top bar (`useTheme.js`) lets it be overridden manually - useful for actually seeing both themes without going into system settings. The override is a `data-theme="light"|"dark"` attribute on `<html>`, persisted to `localStorage`; `index.css` declares each theme's variables three times - the default `:root` block, the `@media (prefers-color-scheme: dark)` block (now scoped to `:root:not([data-theme="light"])` so an explicit light override can suppress it), and an unconditional `:root[data-theme="dark"]` block (an attribute selector, so it outranks the plain `:root` a system-light default lives on) - the standard "respect system preference, but let a manual toggle win" pattern, which needs that duplication since a plain CSS rule can't be shared across a `@media` boundary.
