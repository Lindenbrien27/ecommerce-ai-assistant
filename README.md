...
`Layout.jsx` renders a full-width storefront-style shell (a top identity/category bar, then a second "account" bar with a greeting and a pill nav) instead of the earlier collapsible left sidebar - `#root:has(> .storefront-shell)` zeroes out `#root`'s own padding/centering for exactly this shell, since that padding is still correct for `VerifyPage`'s separate, narrower `.app-shell--centered` card.

Most of the top bar and the account bar's Profile/Wishlist/Saved Address/Change Password pills are intentionally inert: plain `<span>`s, not `<button>`s, with no `onClick`. This app has no catalog, wishlist, or address-book backend for any of them to actually do something - giving them real button semantics with no real behavior would mean either a broken keyboard/AT experience (focusable, announced as a control, does nothing) or fighting that with `tabIndex`/`aria-hidden` on every one. A non-interactive element styled to look clickable is simpler and more honest about what it actually is. **My Order**, **Support Chat**, and **Logout** are the three pills that are real `NavLink`/`button` elements with real behavior - Logout specifically was kept working even though it isn't part of the reference design this shell's style is based on, since a purely decorative Logout would strand anyone signed in with no way back to `/verify`.

The order list's left-hand status filter (All/On shipping/Arrived/Cancelled/Returned) *is* fully functional, unlike the chrome above - it's a client-side filter over already-loaded orders plus real counts computed per status, not decoration, since the data to back it honestly was already there. The one exception is **Returned**: this app's `status` CHECK constraint has no matching value, so that filter's count is always 0 and its list is always empty rather than being quietly mapped onto some other status that isn't actually the same thing.
