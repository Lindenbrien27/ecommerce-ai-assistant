...
function EventCalendarViewSwitcher({
  ...
  return (
    ...
    >
      {/* Tooltip on an overlay-opener: force-closed while the menu is up, so
          it never lingers when focus returns after selection. Radix's
          Tooltip onOpenChange only reports open/closed, not *why* (Base
          UI's `details.reason` has no Radix equivalent), so this no longer
          distinguishes a hover open from a keyboard-focus open - both show
          the tooltip while the menu is closed. */}
...
function EventCalendarNav({
  ...
  const defaultProps = {
    ...
    children: children ?? (
      ...
      // Radix's TooltipProvider only exposes delayDuration - closeDelay/timeout
      // are Base UI-only knobs with no Radix equivalent.
      <TooltipProvider delayDuration={tooltips.delay}>
