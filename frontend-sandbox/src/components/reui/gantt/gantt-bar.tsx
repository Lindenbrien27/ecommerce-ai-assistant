...
function GanttBar<TData = unknown>({
  ...
  const barTree = (
    // Radix's Tooltip onOpenChange only reports open/closed, not *why* (Base
    // UI's `details.reason` has no Radix equivalent), so this no longer
    // distinguishes a hover open from a focus/press open.
    <TooltipProvider delayDuration={500}>
      <Tooltip open={tipOpen && !anyInteracting} onOpenChange={setTipOpen}>
        {trigger}
