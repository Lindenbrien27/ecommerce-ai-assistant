  ...
  return (
    ...
    >
      {/* Tooltip on an overlay-opener: force-closed while the menu is up, so
          ...
          distinguishes a hover open from a keyboard-focus open. */}
...
function GanttNav({ className, render, children, ...props }: GanttNavProps) {
  ...
  const defaultProps = {
    ...
    children: children ?? (
      // Shared provider: first tooltip waits, moving between buttons is
      // instant. Radix's TooltipProvider only exposes delayDuration -
      // closeDelay/timeout are Base UI-only knobs with no Radix equivalent.
